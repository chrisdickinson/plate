...
var Template = function(raw, tag_library, filter_library, parser) {
    if(typeof(raw) !== 'string' || !raw) {
        throw new TypeError("You must pass in a string to render the template.");
    }
    this.raw = raw;
    this.tag_library = tag_library || new libraries.DefaultTagLibrary();
    this.filter_library = filter_library || new libraries.DefaultFilterLibrary();
...
Template.prototype.render = function(context, callback) {
    ...
    if(!this.nodelist) {
        var parser = new this.parser(this.tokens, this.tag_library, this.filter_library);
